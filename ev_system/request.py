...
def schedule():
    ...
    bldg_query = ""
    bldg_name = request.args.get('bldg_name', "-1")
    if bldg_name != "-1":
...
@login_required
def verify():
    station = request.args.get('station_id', None)
    ...
    req = db.execute(
        ...
        ' FROM requests r JOIN slots s on r.slot_id = s.id JOIN stations s2 on s.station_id = s2.id'
        ' WHERE requester_id = ? AND s2.id = ?', [g.user['id'], station]
    ).fetchone()
    ...
    if req is not None:
        status = req['status']
        if status == "notify":
            return start_verified(req['id'])
        elif status == "evacuate":
            return stop_verified(req['id'])
        else:
...
def start_verified(id):
    ...
    system.generate_code(r['station_id'], db)
...
def stop(id):
    ...
    if r['req_status'] != 'charging' and r['req_status'] != 'evacuate':
        flash("Request is not charging", 'error')
        return redirect(url_for('request.index'))
